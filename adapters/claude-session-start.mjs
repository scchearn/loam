...
async function defaultContext({ integrationPath, workspace }) {
  try {
    ...
    await integration.runIntegration(
      ['hook', '--harness', 'claude', '--workspace', workspace],
...
export function createClaudeAdapter({ integrationPath, getContext = defaultContext } = {}) {
...
export async function handleClaudeHook(payload, options) {
  return createClaudeAdapter(options)(typeof payload === 'string' ? JSON.parse(payload) : payload);
}
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(fileURLToPath(import.meta.url))) {
  ...
  process.stdout.write(`${JSON.stringify(await handleClaudeHook(input || '{}'))}\n`);
}
