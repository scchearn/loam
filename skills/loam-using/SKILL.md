...
- **View:** "open loam view", "show me the project view", "let me see the workspace" → open Loam View (below); it is a server, not a skill.
...
Background-spawn `npx @scchearn/loam view "$(pwd)" --no-open` (the packaged Node entry; the native runtime has no `view` verb, and a headless agent has no browser). Read the first stdout line, `Loam View: http://127.0.0.1:<port>/`, and give the user that URL; the port is ephemeral, parse it every time. Do not poll, kill, or read the page yourself; let the harness own the child. It is read-only, loopback-only, and exports nothing. If the launcher reports unsupported Node.js (needs >= 22) or a missing runtime, relay its message and recovery line; do not work around it.
