...
export const RUNTIME_VERSION = '1.1.0-next.0';
...
Three verbs, one job each:
  npx @scchearn/loam install     First-time install; interactive runs can enable optional integrations.
...
Usage:
  npx @scchearn/loam install [--integration <id>]... [--yes] [--dry-run]
...
Commands:
  install     First-time installation of global Loam skills, runtime, harness
              adapters, and shared integration. Interactive runs can enable optional
              integrations; --yes leaves them disabled unless explicitly requested.
              Idempotent — re-running
              repairs a damaged install at the same version; a healthy install is a
              fast no-op.
...
Options:
  ...
  --integration   With setup, or install for an explicit first-run choice, select an optional integration (repeatable).
