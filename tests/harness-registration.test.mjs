...
test('claude and codex carry self-resolving hook shims; cursor invokes the runtime directly', async () => {
  ...
  assert.equal(cursorOwned[0].command, `${JSON.stringify(runtimePath)} hook cursor --event sessionStart`);
...
test('the rendered command tracks the staged runtime rather than a constant', async () => {
  ...
  assert.deepEqual(command(await read(first)), [`${JSON.stringify(first.runtimePath)} hook cursor --event sessionStart`]);
  assert.deepEqual(command(await read(second)), [`${JSON.stringify(second.runtimePath)} hook cursor --event sessionStart`]);
