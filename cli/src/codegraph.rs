...
const GENERATED_HEADER_BYTES: usize = 2 * 1024;
...
fn generated_header(content: &[u8]) -> bool {
    ...
    let header = &content[..content.len().min(GENERATED_HEADER_BYTES)];
    MARKERS
        .into_iter()
        .any(|marker| contains_ascii_case_insensitive(header, marker))
}
