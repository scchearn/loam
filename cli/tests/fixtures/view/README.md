...
| `chronicle/` | T6 events/metrics/signals fixture: `wiki/log.md` headings, a checkpoint chain where the newer checkpoint supersedes the older one, a goal with one valid and one invalid `### YYYY-MM-DD` review heading and a noncanonical `±HHMM` `created_at`, a concept page, an archived page, and a goal-less draft spec. No `wiki/code/` or `src/`, so `code.coverage_percent` exercises the zero-denominator -> unavailable path. |
